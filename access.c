/*
 * The files with access control on open
 */
...
/*
 * The first device is the single-open one, it has an hw structure
 * and an open count
 */
...
/*
 * Next, the device with blocking-open on uid
 */
...
static int scull_w_release(struct inode *inode, struct filp *filp)
{
	int temp;
...
/*
 * Finally the 'cloned' private device. This is trickier because it involves
 * list management and dynamic allocation
 */
...
static int scull_c_open(struct inode *inode, struct filp *filp)
{
...
void scull_access_cleanup(void)
{
	...
	for  (i = 0; i < SCULL_N_ADEVFS; i++) {
		struct scull_dev *dev = scull_access_devs[i].sculldev;
		cdev_del(&dev->cdev);
		scull_trim(scull_release_devs[i].sculldev);
	}
